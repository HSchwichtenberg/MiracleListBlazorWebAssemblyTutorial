...
public partial class Index
{
 ...
 protected override async Task OnInitializedAsync()
 {
  ...
  // --- per ASP.NET Core SignalR eingehende Nachricht: TaskListUpdate
  ...
  await hubConnection.SendAsync("Register", am.Token);
 }
 ...
 public async Task NewCategory_Keyup(KeyboardEventArgs e)
 {
  if (e.Key == "Enter")
  {
   if (!String.IsNullOrEmpty(this.newCategoryName))
   {
    ...
    await hubConnection.SendAsync("CategoryListUpdate", am.Token);
   }
  }
 }
 ...
 public async Task NewTask_Keyup(KeyboardEventArgs e)
 {
  if (e.Key == "Enter")
  {
   if (!String.IsNullOrEmpty(this.newTaskTitle))
   {
    ...
    this.newTaskTitle = "";
    // SignalR-Nachricht senden
    await hubConnection.SendAsync("TaskListUpdate", am.Token, this.category.CategoryID);
   }
  }
 }
